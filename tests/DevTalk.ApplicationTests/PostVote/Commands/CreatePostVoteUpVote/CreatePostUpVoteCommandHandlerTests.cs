...
namespace DevTalk.Application.PostVote.Commands.CreatePostVoteUpVote.Tests;

public class CreatePostUpVoteCommandHandlerTests
{
    [Fact]
    public async Task Handle_WhenPostIdIsNull_ThrowsArgumentNullException()
    {
        // Arrange
        var command = new CreatePostUpVoteCommand(null);
        var unitOfWorkMock = new Mock<IUnitOfWork>();
        var userContextMock = new Mock<IUserContext>();

        var handler = new CreatePostUpVoteCommandHandler(unitOfWorkMock.Object, userContextMock.Object);
    ...
    public async Task Handle_WhenPostNotFound_ThrowsNotFoundException()
    {
        ...
        var command = new CreatePostUpVoteCommand(postId);
    ...
    [Fact]
    public async Task Handle_WhenNoExistingVote_AddsNewUpVote()
    {
        // Arrange
        var postId = "valid_post_id";
        var userId = "user_id";
        ...
        var userContextMock = new Mock<IUserContext>();
        userContextMock.Setup(uc => uc.GetCurrentUser()).Returns(new CurrentUser(userId, "user@email.com", new List<string> { UserRoles.User }));
        ...
        Xunit.Assert.Equal(VoteType.UpVote, post.Votes.First().VoteType);
        unitOfWorkMock.Verify(uow => uow.SaveAsync(), Times.Once);
    }

    [Fact]
    public async Task Handle_WhenExistingUpVote_RemovesVote()
    {
        // Arrange
        var postId = "valid_post_id";
        var userId = "user_id";
        ...
        postVotesRepositoryMock.Setup(repo => repo.Remove(existingVote));
    ...
    [Fact]
    public async Task Handle_WhenExistingDownVote_ChangesToUpVote()
    {
        // Arrange
        var postId = "valid_post_id";
        var userId = "user_id";
